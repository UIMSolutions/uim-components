...
class DUIMButton :DUIMComponent {
	...
	override DVUEComponent toVUEComponent() {
		...
		_templateObj =  BS4Button([":class":"this.classes", ":style":"this.styles",
		"@click":"click", ":aria-pressed":"this.active"], "<slot />");
		return super.toVUEComponent
		.name("UimButton")
    .props("block", `{type:Boolean,default:false}`)
    .props("color", `{type:String,default:"none", validator:value => ["primary", "secondary", "success", "danger", "warning", "info", "light", "dark", "link"].indexOf(value) >= 0}`)
